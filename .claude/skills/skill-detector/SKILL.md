...
Query WorkIQ MCP with 18 signal-extraction prompts every cycle.
...
Targeted Gap Queries (3, NEW v3.2):
16. What content did multiple people create independently this week that covers the same topic? (detects parallel creation waste)
17. What questions were asked of me that I have answered before in a previous week? (detects FAQ deflection candidates with temporal evidence)
18. What meetings generated follow-up artifacts (emails, docs, tasks) within 24 hours? (detects meeting-to-artifact chains)

### Step 2: CLASSIFY -- Map to Pattern Archetypes

19 evidence-backed archetypes derived from 26 cycles of M365 observation:
...
#### Temporal Decay Weighting (NEW v3.2)
...
Current: Eval SME PM BSI CRITICAL (87). 15/week, 5 roles, 10+ query types.
- T1 automate: FAQ deflection (eval-design-advisor, agent-architecture-advisor)
...
Active Gaps:
- Eval decks SGI 90+ (3+ parallel versions across teams)
- Meeting notes SGI 85+ (Loop vs email vs doc formats)
- Status reports SGI 80+ (3 formats: email, Loop page, deck slide)
...
5. Signal-to-Sync-to-Artifact: Email signal -> Focused meeting -> Loop capture (NEW v3.2)
...
### Step 5: ASSESS READINESS -- Skill Readiness Index (NEW v3.2)
...
Current SRI (Cycle 26):

| Pattern | Auto | Value | Evidence | Spec | SRI | Gate |
|---------|------|-------|----------|------|-----|------|
| ado-notification-router | 92 | 88 | 60 | 90 | 83 | BUILD |
| meeting-notes-action-extractor | 88 | 87 | 70 | 85 | 83 | BUILD |
| eval-design-advisor | 72 | 93 | 70 | 80 | 79 | BUILD |
| signal-sync-artifact-orchestrator | 76 | 91 | 80 | 70 | 79 | BUILD |
| weekly-status-report-generator | 83 | 90 | 50 | 75 | 75 | BUILD |
| prd-spec-scaffolder | 84 | 88 | 40 | 60 | 68 | PROTOTYPE |
| agent-architecture-advisor | 68 | 80 | 50 | 60 | 65 | PROTOTYPE |
| compliance-alert-classifier | 89 | 72 | 30 | 70 | 65 | PROTOTYPE |
| deck-review-summarizer | 75 | 78 | 40 | 50 | 61 | DESIGN |

### Step 6: GENERATE -- Output Skill Specs

For SRI >= 75 (BUILD), generate full blueprint: name, version, SRI, triggers (explicit + proactive), context (required + optional M365 sources), pipeline steps, output format/schema/delivery, guardrails, success_metrics, amplifies list.
...
## Build Priority (Cycle 26)

TIER 1 -- BUILD (SRI 75+):
P1 ado-notification-router (SRI 83) -- Auto 92, 30/week, 22.5h total, zero-judgment
P2 meeting-notes-action-extractor (SRI 83) -- Chain head, amplifies 3 downstream
P3 eval-design-advisor (SRI 79) -- BSI CRITICAL relief, highest value (93)
P4 signal-sync-artifact-orchestrator (SRI 79) -- Named pipeline, cross-source chain head
P5 weekly-status-report-generator (SRI 75) -- SGI HIGH, 3 participants

TIER 2 -- PROTOTYPE (SRI 65-74):
P6 prd-spec-scaffolder (SRI 68)
P7 agent-architecture-advisor (SRI 65)
P8 compliance-alert-classifier (SRI 65)

TIER 3 -- DESIGN (SRI 50-64):
P9 deck-review-summarizer (SRI 61)
P10 artifact-distribution-assistant (SRI 55)
P11 org-navigator-skill (SRI 53)

TIER 4 -- WATCH (SRI < 50):
daily-briefing-prep | digest-summarizer | event-content-coordinator | immersion-standup-synthesizer | skill-library-builder
...
Graduation: Candidate->Confirmed = 3 consecutive stable. Confirmed->Mature = composite>=70 for 5+ cycles. Mature->Institutional = 15+ cycles AND evidence density > 70.
Decline: 2 consecutive drops = declining. Absent 3 = demote. Absent 5 = archive.

Current (Cycle 26):
- Institutional (4): ado-notification-triage, meeting-output-packaging, eval-framework-guidance, signal-sync-artifact-pipeline
- Mature (5): weekly-exec-status-assembly, compliance-alert-triage, technical-artifact-distribution, internal-nav-routing, agent-architecture-faq
...
- Candidate (5): evalcon-content-coordination, weekly-status-loop-update, excel-csv-eval-analytics, camp-air-immersion-event, customer-eval-demo-sessions
...
12. SRI Gaming -- high scores on two dimensions do not compensate for zero on others (NEW v3.2)
...
11. Amplification compounds -- correlated patterns yield more than sum
...
16. Temporal decay keeps rankings fresh -- recent evidence wins ties (NEW v3.2)
17. SRI gates build decisions -- high composite without spec clarity = prototype not build (NEW v3.2)
18. Every skill spec must include guardrails and success metrics (NEW v3.2)
...
| 3.1.0 | 25 | Cross-Pattern Amplification Matrix (7 links). Pattern Maturity Lifecycle (5-stage). 19th archetype. Composite rebalanced. 11th anti-pattern. |
| 3.2.0 | 26 | Skill Readiness Index (SRI) with 4 dimensions and BUILD/PROTOTYPE/DESIGN/WATCH gates. Temporal Decay Weighting (0.92 lambda). 3 targeted harvest queries (#16-18). Skill blueprint schema. 5th named pipeline. 12th anti-pattern. 3 new principles. Pipeline expanded to 6 steps. 4 patterns promoted to Institutional. |
