  ...
  let summarySent = false;
  {
    ...
    try {
      ...
      if (!shouldPost) {
        ...
        let feedMessage = "";
        // Try multiple patterns — agent might use different heading styles
        const feedPatterns = [
          /## FEED POST\s*\n([\s\S]*?)(?:\n## |\n---|\s*$)/i,
          /FEED POST\s*\n([\s\S]*?)(?:\n## |\n---|\s*$)/i,
          /blog post[:\s]*\n([\s\S]*?)(?:\n## |\n---|\s*$)/i,
        ];
        for (const pat of feedPatterns) {
          const m = refineSummary.match(pat);
          if (m && m[1].trim().length > 20) {
            feedMessage = m[1].trim();
            break;
          }
        }
        // Fallback: build a rich summary from available data
        if (!feedMessage) {
          const lines = refineSummary
            .split("\n")
            .filter((l: string) => l.trim() && !l.startsWith("#") && !l.startsWith("---") && !l.startsWith("<<<") && !l.startsWith(">>>") && !l.startsWith("==="))
            .slice(0, 8)
            .join("\n")
            .trim()
            .slice(0, 600);
          if (lines.length > 50) {
            ...
            // Last resort: build from structured data
            const focusLabel = cycleNum % 2 === 1 ? "skill-detector" : "dashboard";
            feedMessage = `Cycle ${cycleNum} focused on ${focusLabel}. Tracking ${patCount} patterns — top candidate: ${topName} (score: ${topScoreVal}). ${refineToolCalls} edits made this round.`;
          }
        }
